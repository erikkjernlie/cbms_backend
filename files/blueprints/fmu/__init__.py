...
class P:
    ...
    def start(self, start_time, time_step_input_ref='-1'):
        ...
        print("self.file", self.file, "path", os.path.realpath("./"), "unzipdirectory", fmpy.extract(self.file, os.path.realpath('./')))
    ...
    def step(self, t):
        ...
        if self.dt > 0:
            print("time_step_input_ref", self.time_step_input_ref)
            if self.time_step_input_ref >= 0:
                print("self.fmu setreal", [self.time_step_input_ref], [self.dt])
                self.fmu.setReal([self.time_step_input_ref], [self.dt])
            print("currentCommunicationPoint", self.t, "communicationStepSize", self.dt)
            print("fmu", self.fmu)
