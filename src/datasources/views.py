...
async def datasource_create(request: web.Request):
    ...
    addr = try_get(post, 'address')
    port = try_get(post, 'port')
    output_names = post.getall('output_name')
    output_refs = await try_get_all(post, 'output_ref', int)
    data_type = try_get_validate(post, 'data_type')
    if data_type == "json":
        print("byte format is json")
    else:
        if post.get('catman', ''):
    ...
    if data_type == "json":
        try:
            byte_format = '<' + 'd' * len(output_names) + 'd'
            print("byte_format", byte_format)
            datasource = UdpDatasource(addr, byte_format, output_names, output_refs, time_index, data_type)
...
@routes.get('/datasources/available', name='get_available_datasources')
async def get_available_datasources(request: web.Request):
    datasources = request.app['datasources'].get_available_sources()
    return web.json_response(datasources, dumps=dumps)
...
@routes.get('/datasources/{id}/format', name='datasource_format')
async def datasource_format(request: web.Request):
    """
    List all datasources.
    ...
    data = request.app['datasources'].get_available_sources()[id]
...
async def datasource_subscribe(request: web.Request):
    ...
    if datasource_id not in request.app['datasources']:
        raise web.HTTPBadRequest(reason='Datasource is not running')
    print("before getting topic")
